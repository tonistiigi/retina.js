(function() {
  var DefaultParser, RetinaControl, StaticParser, activate_element, add_parser, clear_parsers, ignore_element, parsers, remove_parser, retina, root, scan, set_manual_mode, set_parser, _active_controls, _active_elements, _current_zoom_level, _get_zoom_level, _ignore_list, _schedule_scan, _walker;
  ...
  scan = function(reset) {
    ...
    if (reset) _walker.currentNode = document.body;
    count = 0;
    while (element = _walker.nextNode()) {
      if (__indexOf.call(_ignore_list, element) >= 0) continue;
      if (__indexOf.call(_active_elements, element) < 0) activate_element(element);
      if (++count > 50) {
